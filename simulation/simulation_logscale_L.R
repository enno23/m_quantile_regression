...
mqs1 = array( dim=c(H,10,S ))
mqs2 = array( dim=c(H,10,S ))
...
for( s in 1:S){
  pop = lapply(sim(setup,R=1), function(X){return (X) })[[1]]
  smp=NULL
   for(i in 1:Domains){
     smp = rbind(smp,      sample_n(pop[which(pop$idD==i),], sample_size[i] ))
     }
true_est <- direct("y", smp_data = pop, smp_domains = "idD", seed = NULL)
mq_est <- mq_sae(y~x, smp_data = smp, pop_data = pop, smp_domains = "idD", pop_domains = "idD",seed=NULL, L=50, B=50, grid=q, MSE=F, S=50, parallel_mode = "socket", cpus=3)
mq_est1 <- mq_sae(y~x, smp_data = smp, pop_data = pop, smp_domains = "idD", pop_domains = "idD",seed=NULL, L=100, B=50, grid=q, MSE=F, S=50, parallel_mode = "socket", cpus=3)
mq_est2 <- mq_sae(y~x, smp_data = smp, pop_data = pop, smp_domains = "idD", pop_domains = "idD",seed=NULL, L=150, B=50, grid=q, MSE=F, S=50, parallel_mode = "socket", cpus=3)
...
mqs1[,,s] <- as.matrix(mq_est1$ind[-1])
mqs2[,,s] <- as.matrix(mq_est2$ind[-1])
...
objects   = c("true", "mqs","mqs1","mqs2", "ebs", "dirs")
save(list = objects, file = "PLEASE_SET_WD/simulation/log_point_L.RData")
