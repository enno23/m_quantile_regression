...
libraries = c("saeSim", "roxygen2","devtools", "mvtnorm","doParallel", "nlme", "ggplot2", "ggthemes", "emdi", "reshape2","plyr","quantreg", "animation", "dplyr", "devtools", "data.table", "quantreg", "rmutil", "VGAM", "expectreg", "mboost", "multcomp")
...
mq_est <- mq_sae(y~x, smp_data = smp, pop_data = pop, smp_domains = "idD", pop_domains = "idD",seed=NULL, L=50, B=B, grid=q, MSE=T, S=50, parallel_mode = "socket", cpus=24, smoothed=F)
...
eb_est <- emdi::ebp(y~x, smp_data = smp, pop_data = pop, smp_domains = "idD", pop_domains = "idD",seed=NULL, L=50, B=B, MSE=T, parallel_mode = "socket", cpus=24)
#dir_est <- emdi::direct("y", smp_data = smp, smp_domains = "idD", var=T, B=B, seed=NULL)
...
#dirs[,,s] <- as.matrix(dir_est$MSE[-1])
...
objects   = c("true", "mqs","MSE_mq", "MSE_eb", "ebs")
#save(list = objects, file = "PLEASE_SET_WD/simulation/normal_mse.RData")
